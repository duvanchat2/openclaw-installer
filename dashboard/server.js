...
const auditLogPath = '/opt/openclaw-dashboard/data/audit.log';
const credentialsFile = '/opt/openclaw-dashboard/data/credentials.json';
const mfaSecretFile = '/opt/openclaw-dashboard/data/mfa-secret.txt';
...
const vaultFile=path.join(dataDir,'credentials-vault.json');
const costAlertsFile=path.join(dataDir,'cost-alerts.json');
function getVault(){try{return JSON.parse(fs.readFileSync(vaultFile,'utf8'))}catch{return{providers:[],mcps:[]}}}
function saveVault(d){fs.writeFileSync(vaultFile,JSON.stringify(d,null,2))}
function getCostAlerts(){try{return JSON.parse(fs.readFileSync(costAlertsFile,'utf8'))}catch{return{dailyLimit:5,monthlyLimit:100,alertThreshold:0.8}}}
function saveCostAlerts(d){fs.writeFileSync(costAlertsFile,JSON.stringify(d,null,2))}
function syncVaultToSystem(vault){
  try{
    const dcPath='/opt/openclaw/docker-compose.yml';
    const authPath='/opt/openclaw/data/agents/main/agent/auth-profiles.json';
    let dc=fs.readFileSync(dcPath,'utf8');
    const authProfiles={};
    const envMap={};
    for(const p of vault.providers||[]){
      if(!p.apiKey||p.apiKey.includes('...'))continue;
      const name=p.name.toLowerCase();
      if(name==='google'||name==='gemini'){envMap['GOOGLE_API_KEY']=p.apiKey;authProfiles['google']={apiKey:p.apiKey}}
      else if(name==='zai'||name==='zhipu'){envMap['ZAI_API_KEY']=p.apiKey;authProfiles['zai']={apiKey:p.apiKey}}
      else if(name==='openrouter'){envMap['OPENROUTER_API_KEY']=p.apiKey;authProfiles['openrouter']={apiKey:p.apiKey}}
      else if(name==='anthropic'||name==='claude'){envMap['ANTHROPIC_API_KEY']=p.apiKey;authProfiles['anthropic']={apiKey:p.apiKey}}
      else if(name==='openai'){envMap['OPENAI_API_KEY']=p.apiKey;authProfiles['openai']={apiKey:p.apiKey}}
      else if(name==='notion'){envMap['NOTION_API_KEY']=p.apiKey}
      else{envMap[name.toUpperCase()+'_API_KEY']=p.apiKey;authProfiles[name]={apiKey:p.apiKey}}
    }
    for(const[key,val]of Object.entries(envMap)){
      const regex=new RegExp('- '+key+'=.*','g');
      if(dc.match(regex)){dc=dc.replace(regex,'- '+key+'='+val)}
      else{dc=dc.replace(/(environment:\n)/,'$1      - '+key+'='+val+'\n')}
    }
    fs.writeFileSync(dcPath,dc);
    if(Object.keys(authProfiles).length>0){
      const dir=require('path').dirname(authPath);
      fs.mkdirSync(dir,{recursive:true});
      let existing={};try{existing=JSON.parse(fs.readFileSync(authPath,'utf8'))}catch{}
      Object.assign(existing,authProfiles);
      fs.writeFileSync(authPath,JSON.stringify(existing,null,2));
      fs.chmodSync(authPath,'0666');
    }
    const{exec}=require('child_process');
    exec('cd /opt/openclaw && docker-compose down && docker-compose up -d',{timeout:60000},(err,stdout)=>{
      if(err)console.error('Vault sync restart error:',err.message);
      else console.log('Vault synced, OpenClaw restarted');
    });
  }catch(e){console.error('syncVaultToSystem error:',e.message)}
}
function getSkillsDetailed(){const s=[];const sDir=path.join(WORKSPACE_DIR,'skills');const bDir='/usr/local/lib/node_modules/openclaw/skills';try{if(fs.existsSync(sDir)){for(const e of fs.readdirSync(sDir)){const ep=path.join(sDir,e);if(fs.statSync(ep).isDirectory()){const sm=path.join(ep,'SKILL.md');if(fs.existsSync(sm)){const ct=fs.readFileSync(sm,'utf8');const st=fs.statSync(sm);let desc='';const m=ct.match(/description:\s*(.+)/);if(m)desc=m[1].trim();s.push({name:e,description:desc,source:'workspace',size:st.size,modified:st.mtimeMs,enabled:true})}}}}}catch{}try{if(fs.existsSync(bDir)){for(const e of fs.readdirSync(bDir)){if(!s.find(x=>x.name===e)){const ep=path.join(bDir,e);if(fs.statSync(ep).isDirectory()){const sm=path.join(ep,'SKILL.md');if(fs.existsSync(sm)){const st=fs.statSync(sm);s.push({name:e,description:'Bundled skill',source:'bundled',size:st.size,modified:st.mtimeMs,enabled:true})}}}}}}catch{}return s}
function getMemoryDetailed(){const r={memoryMd:null,dailyNotes:[],soulMd:null,userMd:null,totalSize:0};const mDir=path.join(WORKSPACE_DIR,'memory');try{const p=path.join(WORKSPACE_DIR,'MEMORY.md');if(fs.existsSync(p)){const s=fs.statSync(p);const ct=fs.readFileSync(p,'utf8');r.memoryMd={size:s.size,modified:s.mtimeMs,lines:ct.split('\n').length,preview:ct.substring(0,500)};r.totalSize+=s.size}}catch{}try{const p=path.join(WORKSPACE_DIR,'SOUL.md');if(fs.existsSync(p)){const s=fs.statSync(p);const ct=fs.readFileSync(p,'utf8');r.soulMd={size:s.size,modified:s.mtimeMs,lines:ct.split('\n').length,preview:ct.substring(0,500)};r.totalSize+=s.size}}catch{}try{const p=path.join(WORKSPACE_DIR,'USER.md');if(fs.existsSync(p)){const s=fs.statSync(p);const ct=fs.readFileSync(p,'utf8');r.userMd={size:s.size,modified:s.mtimeMs,lines:ct.split('\n').length,preview:ct.substring(0,500)};r.totalSize+=s.size}}catch{}try{if(fs.existsSync(mDir)){for(const f of fs.readdirSync(mDir).sort().reverse()){if(f.endsWith('.md')){const fp=path.join(mDir,f);const s=fs.statSync(fp);r.dailyNotes.push({name:f,size:s.size,modified:s.mtimeMs});r.totalSize+=s.size}}}}catch{}return r}
function getSubAgents(){const a=[];const aDir=path.join(OPENCLAW_DIR,'agents');try{if(fs.existsSync(aDir)){for(const ag of fs.readdirSync(aDir)){const ap=path.join(aDir,ag);if(fs.statSync(ap).isDirectory()){let sc=0,la=0;try{const sf=path.join(ap,'sessions','sessions.json');const d=JSON.parse(fs.readFileSync(sf,'utf8'));sc=Object.keys(d).length;for(const v of Object.values(d)){if(v.updatedAt>la)la=v.updatedAt}}catch{}a.push({id:ag,sessions:sc,lastActive:la})}}}}catch{}return a}
  ...
  if (req.url.startsWith('/api/')) {
    ...
    if (req.url === '/api/action/restart-openclaw' && req.method === 'POST') {
      try {
        ...
        exec('cd /opt/openclaw && docker-compose restart openclaw', (err) => {});
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: true }));
      } catch (e) {
        res.writeHead(500, { 'Content-Type': 'application/json' });
    ...
    if (req.url === '/api/action/restart-dashboard' && req.method === 'POST') {
      try {
        ...
        setTimeout(() => {
          exec('systemctl restart openclaw-dashboard', (err) => {});
        }, 2000);
        res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if(req.url==='/api/skills-detailed'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(getSkillsDetailed()));return}
if(req.url.startsWith('/api/skill-content?')){const p=new URL(req.url,'http://localhost').searchParams;const n=(p.get('name')||'').replace(/[^a-zA-Z0-9_-]/g,'');const sp=path.join(WORKSPACE_DIR,'skills',n,'SKILL.md');try{if(fs.existsSync(sp)){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({name:n,content:fs.readFileSync(sp,'utf8')}))}else{res.writeHead(404,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'Not found'}))}}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}return}
if(req.url==='/api/skill-save'&&req.method==='POST'){let b='';req.on('data',c=>b+=c);req.on('end',()=>{try{const{name,content}=JSON.parse(b);const n=name.replace(/[^a-zA-Z0-9_-]/g,'');const d=path.join(WORKSPACE_DIR,'skills',n);fs.mkdirSync(d,{recursive:true});fs.writeFileSync(path.join(d,'SKILL.md'),content);res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({success:true}))}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}});return}
if(req.url==='/api/skill-delete'&&req.method==='POST'){let b='';req.on('data',c=>b+=c);req.on('end',()=>{try{const{name}=JSON.parse(b);const n=name.replace(/[^a-zA-Z0-9_-]/g,'');const d=path.join(WORKSPACE_DIR,'skills',n);if(fs.existsSync(d))fs.rmSync(d,{recursive:true});res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({success:true}))}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}});return}
if(req.url==='/api/openclaw-version'){
  const{exec}=require('child_process');
  exec('docker exec openclaw openclaw --version 2>/dev/null',{timeout:10000},(e,out)=>{
    const current=out?out.trim():'unknown';
    res.writeHead(200,{'Content-Type':'application/json'});
    res.end(JSON.stringify({current:current}));
  });return;
}
if(req.url==='/api/openclaw-update'&&req.method==='POST'){
  const{exec}=require('child_process');
  let b='';req.on('data',c=>b+=c);req.on('end',()=>{
    try{
      const{version}=JSON.parse(b);
      const v=version.replace(/[^0-9.]/g,'');
      if(!v){res.writeHead(400,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'Invalid version'}));return}
      const df='/opt/openclaw/Dockerfile';
      let d=fs.readFileSync(df,'utf8');
      d=d.replace(/openclaw@[0-9.]+/,'openclaw@'+v);
      fs.writeFileSync(df,d);
      res.writeHead(200,{'Content-Type':'application/json'});
      exec('cd /opt/openclaw && docker-compose down && docker build --no-cache -t openclaw:latest . && docker-compose up -d',{timeout:600000},(err,stdout,stderr)=>{});
      res.end(JSON.stringify({success:true,message:'Updating to '+v+'. This takes ~5 minutes. Check /api/openclaw-version after.'}));
...
if(req.url==='/api/backup'&&req.method==='POST'){
  const{exec}=require('child_process');
  res.writeHead(200,{'Content-Type':'application/json'});
  exec('/opt/openclaw-dashboard/backup.sh',{timeout:60000},(err,stdout,stderr)=>{
    if(err){res.end(JSON.stringify({success:false,error:err.message}))}
    else{res.end(JSON.stringify({success:true,output:stdout.trim()}))}
  });return;
}
if(req.url==='/api/vault-env'){
  try{
    const dcPath='/opt/openclaw/docker-compose.yml';
    ...
    for(const line of lines){
      ...
      if(k==='NODE_ENV'||k==='TZ')continue;
      const name=k.replace('_API_KEY','').replace('_BOT_TOKEN','').replace('_TOKEN','').toLowerCase();
      envs[name]={key:k,value:v.substring(0,8)+'...'+v.slice(-4)};
    }
    res.writeHead(200,{'Content-Type':'application/json'});
...
if(req.url==='/api/vault'){res.writeHead(200,{'Content-Type':'application/json'});const v=getVault();const m=JSON.parse(JSON.stringify(v));for(const p of m.providers||[]){if(p.apiKey)p.apiKey=p.apiKey.substring(0,8)+'...'+p.apiKey.slice(-4)}for(const p of m.mcps||[]){if(p.token)p.token=p.token.substring(0,8)+'...'+p.token.slice(-4)}res.end(JSON.stringify(m));return}
if(req.url==='/api/vault-save'&&req.method==='POST'){let b='';req.on('data',c=>b+=c);req.on('end',()=>{try{const d=JSON.parse(b);const ex=getVault();for(const p of d.providers||[]){if(p.apiKey&&p.apiKey.includes('...')){const o=(ex.providers||[]).find(x=>x.name===p.name);if(o)p.apiKey=o.apiKey}}for(const m of d.mcps||[]){if(m.token&&m.token.includes('...')){const o=(ex.mcps||[]).find(x=>x.name===m.name);if(o)m.token=o.token}}saveVault(d);syncVaultToSystem(d);res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({success:true,synced:true}))}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}});return}
if(req.url==='/api/cost-alerts'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(getCostAlerts()));return}
if(req.url==='/api/cost-alerts-save'&&req.method==='POST'){let b='';req.on('data',c=>b+=c);req.on('end',()=>{try{const d=JSON.parse(b);saveCostAlerts(d);res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({success:true}))}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}});return}
if(req.url==='/api/memory-detailed'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(getMemoryDetailed()));return}
if(req.url.startsWith('/api/memory-content?')){const p=new URL(req.url,'http://localhost').searchParams;const f=p.get('file')||'';let fp='';if(f==='MEMORY.md')fp=path.join(WORKSPACE_DIR,'MEMORY.md');else if(f==='SOUL.md')fp=path.join(WORKSPACE_DIR,'SOUL.md');else if(f==='USER.md')fp=path.join(WORKSPACE_DIR,'USER.md');else if(f.match(/^memory\/[0-9-]+\.md$/))fp=path.join(WORKSPACE_DIR,f);if(fp&&fs.existsSync(fp)){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({file:f,content:fs.readFileSync(fp,'utf8')}))}else{res.writeHead(404,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'Not found'}))}return}
if(req.url==='/api/memory-save'&&req.method==='POST'){let b='';req.on('data',c=>b+=c);req.on('end',()=>{try{const{file,content}=JSON.parse(b);let fp='';if(file==='MEMORY.md')fp=path.join(WORKSPACE_DIR,'MEMORY.md');else if(file==='SOUL.md')fp=path.join(WORKSPACE_DIR,'SOUL.md');else if(file==='USER.md')fp=path.join(WORKSPACE_DIR,'USER.md');if(fp){fs.writeFileSync(fp,content);res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({success:true}))}else{res.writeHead(400,{'Content-Type':'application/json'});res.end(JSON.stringify({error:'Invalid file'}))}}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}});return}
if(req.url==='/api/agents'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(getSubAgents()));return}
if(req.url==='/api/openclaw-config'){try{const cp=path.join(OPENCLAW_DIR,'openclaw.json');const cfg=JSON.parse(fs.readFileSync(cp,'utf8'));if(cfg.gateway&&cfg.gateway.auth&&cfg.gateway.auth.token)cfg.gateway.auth.token=cfg.gateway.auth.token.substring(0,6)+'...';res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify(cfg))}catch(e){res.writeHead(500,{'Content-Type':'application/json'});res.end(JSON.stringify({error:e.message}))}return}
